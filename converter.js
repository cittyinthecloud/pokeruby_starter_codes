const seeds = [
    0x09F4FBBD, 0x9681884A, 0x352027E9, 0xF3DEE5A7
]

const table1 = [
    ];

const table2 = [
    
];

const RUBY_10 = 0;
const SAPPHIRE_10 = 1;
const baseaddrs = [
    0x641FBB62, // Ruby 1.0
    0x641FBB8C, // Sapphire 1.0
    0x641FBB6E, // Ruby 1.1
    0x641FBB9E, // Sapphire 1.1 & 1.2
    0x641FBB70, // Ruby 1.2
];
const baseval = 0x10000000;
const u32 = 0xFFFFFFFF;

function get_code_for_pokemon(game, slot, index) {
    const targetaddr = baseaddrs[game] + slot;
    const targetval = baseval | index;

    let part1 = targetaddr;
    let part2 = targetval;

    let sum = 0;
    const delta = 0x9E3779B9;

    for (let i = 0; i < 32; i++) {
        sum = sum + delta & u32;
        part1 = part1 + ((part2 << 4) + seeds[0] & u32 ^ part2 + sum & u32 ^ (part2 >>> 5) + seeds[1] & u32) & u32;
        part2 = part2 + ((part1 << 4) + seeds[2] & u32 ^ part1 + sum & u32 ^ (part1 >>> 5) + seeds[3] & u32) & u32;
    }

    convert = new Uint32Array(2)
    convert[0] = part1;
    convert[1] = part2;

    codepart1 = (u32 + part1 + 1).toString(16).slice(-8).padStart(8, '0').toUpperCase()
    codepart2 = (u32 + part2 + 1).toString(16).slice(-8).padStart(8, '0').toUpperCase()

    return codepart1 + " " + codepart2;
}

console.log(get_code_for_pokemon(RUBY_10, 0, 1))

$(() => {
    $("#generate_button").click(() => {
        $("#code").text(get_code_for_pokemon(parseInt($("#gamerev").val()), parseInt($("#slot").val()), parseInt($("#pkmnindex").val())));
    })
})
